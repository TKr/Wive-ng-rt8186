...
struct svc_sock {
	...
	int			(*sk_recvfrom)(struct svc_rqst *);
	int			(*sk_sendto)(struct svc_sock *, struct svc_buf *);
	...
	/* send stuff */
	spinlock_t		sk_sendlk;
	struct svc_buf *	sk_sendq;	/* send-queue of resbuf's */
	unsigned int		sk_sendstate;
	/* private TCP part */
	unsigned int		sk_reclen;	/* length of record */
	unsigned int		sk_tcplen;	/* current read length */
	struct sockaddr_in	sk_raddr;	/* peer address */
