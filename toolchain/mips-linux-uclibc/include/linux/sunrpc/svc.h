...
struct svc_buf {
	struct svc_buf *	prev; 	/* svc_sock send queue */
	struct svc_buf *	next;
	u32 *			area;	/* allocated memory */
	u32 *			base;	/* base of RPC datagram */
	unsigned int		buflen;	/* total length of buffer */
	u32 *			buf;	/* read/write pointer */
	unsigned int		len;	/* current end of buffer */

	unsigned int		sent;   /* number of bytes sent */

	/* UDP responses should have peer addresses */
	struct sockaddr_in	raddr;	/* peer address */
...
struct svc_buf *   svc_resbuf_alloc(struct svc_serv *);
int		   svc_resbuf_free(struct svc_buf *);
