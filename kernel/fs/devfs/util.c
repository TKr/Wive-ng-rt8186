...
void devfs_dealloc_major (char type, int major)
{
    ...
    spin_unlock (&list->lock);
    if (!was_set)
	printk (KERN_ERR "(): major %d was already free\n",
		major);
...
void devfs_dealloc_devnum (char type, kdev_t devnum)
{
	...
	up (semaphore);
	if (!was_set)
	    printk ( KERN_ERR "(): device %s was already free\n",
		     kdevname (devnum) );
	return;
    }
    up (semaphore);
    printk ( KERN_ERR "(): major for %s not previously allocated\n",
	     kdevname (devnum) );
...
void devfs_dealloc_unique_number (struct unique_numspace *space, int number)
{
    ...
    up (&space->semaphore);
    if (!was_set)
	printk (KERN_ERR "(): number %d was already free\n",
		number);
