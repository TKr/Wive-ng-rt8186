 ...
 *
 * $Id: doc2001.c,v 1.7 2004/12/03 04:33:37 jimmylin Exp $
 */
...
#if defined(CONFIG_RTK_MTD_ROOT) || defined(CONFIG_RTK_MTD_PARTITION)
...
#if defined(CONFIG_RTK_MTD_ROOT) || defined(CONFIG_RTK_MTD_PARTITION)
                RTL8186PartitionCount = RTL8186_ROOT_Seek(mtd,rtl8186_partitions);
		add_mtd_partitions(mtd, rtl8186_partitions,  RTL8186PartitionCount);
#else
		add_mtd_device(mtd);
#endif
		return;
...
//	unsigned long docptr = this->virtadr;
	...
	if (from >= this->totlen)
		return -EINVAL;
// Patched by dgreen - chack read 1 byte - old version read at least 3 bytes
// In any case trying to read even bytes gives much more 2 bytes at read
//	for(i=0; i< len; i+=2)
	for(i=1; i< len; i+=2)
	{
...
// Patched by dgreen. Use usigned short - not unsinged log of cource
	if (len & 0x01)
	{
		tmp = *(volatile unsigned short *)(docptr);
        *(unsigned char *)buf = (tmp >> 8) & 0xff;
...
// Added by dgreen. Bootloader protection
        if(to < flash_hw_start)
	 {
	  printk("Eeror flash wite: bootloader protected !\n");
	  return -1;
	 }

//printk("KERNEL: writing 0x%06X bytes to 0x%08X\n",(int)len,(int)to);
...
// david
//	printk("going to write len=%08x,to =%08x\n", len, to);
...
int doc_erase (struct mtd_info *mtd, struct erase_info *instr)
{
	...
	i = 0;
        ...
        */
        ...
        i--;
//printk("Founded start erase region %d from %d\n",i,mtd->numeraseregions);
...
//printk("Founded end erase region %d from %d\n",i,mtd->numeraseregions);
	...
	while (len) {
//printk("In errases loop with bm=%X\n",flash_write_flag);
...
// Added by dgreen - check on bootloader erase 
        if(addr < flash_hw_start)
	  {
	   printk("Error flash errase: bootloader protected !\n");
	   return -1;
	  }
	...
	else if (this->probe_state == PROBE_ACTIVE)
	{
		...
		else
		{
			...
			else if (this->probe_counter >=5)
			{
				 ...
				 *(volatile unsigned short *)(0xbfc00000 + 0x555 * 2) = 0xaa;
				 *(volatile unsigned short *)(0xbfc00000 + 0x2aa * 2) = 0x55;
				 *(volatile unsigned short *)(0xbfc00000 + 0x555 * 2) = 0x80;
			         *(volatile unsigned short *)(0xbfc00000 + 0x555 * 2) = 0xaa;
			         *(volatile unsigned short *)(0xbfc00000 + 0x2aa * 2) = 0x55;

#ifdef COMPACK_SETTING
				 *(volatile unsigned short *)(0xbfc00000 + flash_hw_start ) = 0x30;
#else
			         *(volatile unsigned short *)(0xbfc00000 + 0x8000 ) = 0x30;
#endif
			   	 for(retlen=0 ; retlen <=0xffffff0; retlen++)
			         {
#ifdef COMPACK_SETTING
					if (*(volatile unsigned short *)(0xbfc00000 + flash_hw_start)==0xffff)
#else
			         	if (*(volatile unsigned short *)(0xbfc08000)==0xffff)
#endif
				        	break;
			         }
