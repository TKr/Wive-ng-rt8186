 ...
 *
 * $Id: mtdblock.c,v 1.4 2004/12/03 04:33:37 jimmylin Exp $
 *
...
int flash_write_flag;		// 1: hw setting 2: default setting, 4: current setting, 8: system image
//-----------------------
			...
			int len, const char *buf)
{
	 ...
	 */
//printk("ERASE_WRITE at %d bytes %d\n",(int)pos,len);
...
static int write_cached_data (struct mtdblk_dev *mtdblk)
{
	...
	//printk("mtdblock: writing cached data for \"%s\" "
	//		"at 0x%lx, size 0x%x\n", mtd->name,
	//		mtdblk->cache_offset, mtdblk->cache_size);
...
//---------------------
	...
	if (!sect_size)
	       {
	        printk("MTDBLOCK: Clear writes\n");
	        retlen=len;
	        return erase_write (mtd, pos, len, buf);
	       }    	
//		return MTD_WRITE (mtd, pos, len, &retlen, buf);
	...
	while (len > 0) {
		...
		if (size == sect_size) { //printk("MTDBLOCK: Whole sector\n");
			/*
