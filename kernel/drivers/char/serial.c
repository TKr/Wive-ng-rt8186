			 ...
			 struct termios *old_termios)
{
	...
	serial_out(info, UART_LCR, lcr | UART_LCR_DLAB);   /* DLAB=1 */
	serial_out(info, UART_DLM, div >> 8);
	serial_out(info, UART_DLL, div & 0xff);
	// shit with 1/2 stop bits
	if (cflag & CSTOPB)
	    lcr |= UART_LCR_STOP;
	else
	    lcr &= ~UART_LCR_STOP;    
	serial_out(info, UART_LCR, lcr);   /* restore LCR (and DLAB) */       
	restore_flags(flags);
