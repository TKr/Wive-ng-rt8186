...
#else
	entry.srcPortL 			= mCast_t->port& 0x1; /* 1st bit */
	entry.srcPortH 			= mCast_t->port>>1;   /* 2,3rd bits */

	entry.srcPortExt 		= mCast_t->port >>3; /*MSB 4th bit*/
#endif
...
#ifdef CONFIG_HW_MULTICAST_TBL_DEBUG
#ifndef CONFIG_RTL865X_LIGHT_ROMEDRV
...
int32 rtl865xC_dumpAsicCounter(void)
{
	...
	for ( i = 0; i <= RTL8651_PORT_NUMBER; i++ )
	{
		...
		panic_printk("Rx %u * 2^32 + %u Bytes, Rx %u Pkts, Drop %u pkts, Rx Pause %u pkts\n", 
		...
		panic_printk("Tx %u * 2^32 + %u Bytes, Tx %u Pkts Tx Pause %u pkts\n", 
...
int32 rtl8651_setAsicAclRule2(uint32 index, _rtl8651_tblDrvAclRule_t *rule) 
{
	static rtl865xc_tblAsic_aclTable_t    entry;
...
void addACLRule(int index)
{
	static _rtl8651_tblDrvAclRule_t aclt;
	...
	rtl8651_setAsicAclRule2(index, &aclt);
}
#endif
...
int8 rtl8651_tblAsicDrv_Id[] = "$Id: rtl865xC_tblAsicDrv.c,v 1.14 2009/07/23 02:34:34 joeylin Exp $";
