		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	case PPPIOCSMRU:
		if (get_user(val, (int *) arg))
			break;
	        ppp->mru_alloc = ppp->mru = val; /* new mru */
                if (ppp->mru_alloc < PPP_MRU) /* new mru */
...
ppp_decompress_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	else 
	{
    		...
    		#ifdef CONFIG_PPP_DEBUG
    		printk(KERN_ERR "NOT MPPE FRAME\n");
    		#endif
	}
...
ppp_create_interface(int unit, int *retp)
{
	...
	ppp->file.index = unit;
	ppp->mru = PPP_MRU;
	ppp->mru_alloc = PPP_MRU; /* alloc memory for PPP recive buffers */
