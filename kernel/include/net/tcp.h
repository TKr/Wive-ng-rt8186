...
static __inline__ int tcp_prequeue(struct sock *sk, struct sk_buff *skb)
{
	...
	if (tp->ucopy.task) {
		...
		if (tp->ucopy.memory > sk->rcvbuf) {
			...
			if (sk->lock.users) out_of_line_bug();
	...
	__u8 *rcv_wscale)
{
	...
	if (wscale_ok) {
                ...
                maxw = max_t(u32, sysctl_tcp_rmem[2], sysctl_rmem_max);
...
static __inline__ int tcp_use_frto(const struct sock *sk)
{
	...
	return (sysctl_tcp_frto && tp->send_head &&
		before(tp->snd_nxt, tp->snd_una + tp->snd_wnd));
}
