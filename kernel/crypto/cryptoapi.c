/* $Id: cryptoapi.c,v 1.16 2002/07/18 02:50:38 kyle Exp $
 *
 * crypto/cryptoapi.c
 *
...
static int
default_hmac(struct digest_context *cx, u8 *key, int key_len,
	     u8 *in, int size, u8 *hmac)
{
...
static int
default_hmac_atomic(struct digest_context *cx, u8 *key, int key_len,
		    u8 *in, int size, u8 *hmac)
{
