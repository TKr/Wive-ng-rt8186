 ...
 *
 * RCSID $Id: ttodata.c,v 1.6 2002/06/09 21:10:43 mcr Exp $
 */
#include "internal.h"
#include "freeswan.h"

/* converters and misc */
static int unhex(const char *, int *, char *, size_t, unsigned int);
static int unb64(const char *, int *, char *, size_t, unsigned int);
static int untext(const char *, int *, char *, size_t,unsigned int);
...
unsigned int flags;
{
	...
	int (*decode)(const char *, int *, char *, size_t, unsigned int);
	char *stop;
	int ndone;
	int i;
	int underscoreok;
	...
	/* proceed */
	ndone = 0;
	while (srclen >= min_ingroup) {
		nbytes = (*decode)(src, &ingroup, buf, sizeof(buf), skipSpace);
		switch (nbytes) {
		case BADCH0:
		case BADCH1:
		case BADCH2:
 ...
 */
static int		/* number of result bytes, or error code */
unb64(src, ingroupP, dst, dstlen, skipSpace)
const char *src;	/* known to be full length */
int *ingroupP;
char *dst;
size_t dstlen;
 ...
 */
static int		/* number of result bytes, or error code */
untext(src, ingroupP, dst, dstlen, skipSpace)
const char *src;	/* known to be full length */
int *ingroupP;
char *dst;
size_t dstlen;		/* not large enough is a failure */
