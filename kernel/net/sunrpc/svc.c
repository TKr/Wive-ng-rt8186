...
extern kmem_cache_t *rpc_rbcachep;
...
struct svc_buf *
svc_resbuf_alloc(struct svc_serv *serv)
{
	struct svc_buf *resbufp;
	
	if (!(resbufp = kmem_cache_alloc(rpc_rbcachep, SLAB_NFS)))
		return NULL;

	memset(resbufp, 0, sizeof (struct svc_buf));

	if (!svc_init_buffer(resbufp, serv->sv_bufsz)) {
		kmem_cache_free(rpc_rbcachep, resbufp);
		return NULL;
	}

	dprintk("rpc: allocated resbuf %p for %s\n", resbufp, serv->sv_name);
	return resbufp;
}

int
svc_resbuf_free(struct svc_buf *bufp)
{
	if (bufp) {
		svc_release_buffer(bufp);
		kmem_cache_free(rpc_rbcachep, bufp);
		dprintk("rpc: freed resbuf %p\n", bufp);
	}
	return 0;
}
