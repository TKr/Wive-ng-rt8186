...
#define SVC_TCP_DEFAULT_SOCKSIZE	(16*35000)
#define SVC_UDP_DEFAULT_SOCKSIZE	(16*35000)
#define SVC_MIN_WRITE_SPACE	(35000)

static struct svc_sock *svc_setup_socket(struct svc_serv *, struct socket *,
					 int *errp, int pmap_reg);
static void		svc_udp_data_ready(struct sock *, int);
static int		svc_udp_recvfrom(struct svc_rqst *);
static int		svc_udp_sendto(struct svc_sock *, struct svc_buf *);
static int		svc_empty_sendq(struct svc_sock *);
...
static void
svc_sock_close(struct svc_sock *svsk)
{
	struct svc_buf  *resbufp;

	dprintk("svc: releasing dead socket %p\n", svsk);
	/* Flush out the send queue first */
	spin_lock(&svsk->sk_sendlk);
	while ((resbufp = svsk->sk_sendq) != NULL) {
		rpc_remove_list(&svsk->sk_sendq, resbufp);
		svc_resbuf_free(resbufp);
	}
	spin_unlock(&svsk->sk_sendlk);
 ...
 */
static int
svc_sendto(struct svc_sock *svsk, struct svc_buf *bufp, struct iovec *iov, int nr)
{
	mm_segment_t	oldfs;
	...
	if (sock->type == SOCK_STREAM) {
		msg.msg_name = &svsk->sk_raddr;
		msg.msg_namelen = sizeof (svsk->sk_raddr);
	} else {
		msg.msg_name    = &bufp->raddr;
		msg.msg_namelen = sizeof(bufp->raddr);
	}
	...
	dprintk("svc: socket %p sendto([%p %Zu... ], %d, %d) = %d\n",
			svsk, iov[0].iov_base, iov[0].iov_len, nr, buflen, len);
...
static void
svc_udp_write_space(struct sock *sk)
{
	struct svc_sock *svsk;
	struct socket *sock;
	...
	if (sock_wspace(sk) < min_t(int, sk->sndbuf,SVC_MIN_WRITE_SPACE))
		return;

	set_bit(SK_WSPACE, &svsk->sk_sendstate);

	if (!test_bit(SK_INSEND, &svsk->sk_sendstate)) {
		spin_lock_bh(&svsk->sk_lock);
		svc_sock_enqueue(svsk);
		spin_unlock_bh(&svsk->sk_lock);
	}
...
svc_udp_recvfrom(struct svc_rqst *rqstp)
{
	...
	memcpy(&rqstp->rq_resbuf->raddr, &rqstp->rq_addr, 
	       sizeof (rqstp->rq_resbuf->raddr));
...
static int
svc_udp_sendto(struct svc_sock *svsk, struct svc_buf *bufp)
{
	int		error;
	...
	error = svc_sendto(svsk, bufp, bufp->iov, bufp->nriov);
...
svc_udp_init(struct svc_sock *svsk)
{
	...
	svsk->sk_sk->write_space = svc_udp_write_space;
	svsk->sk_recvfrom = svc_udp_recvfrom;
	svsk->sk_sendto = svc_udp_sendto;
	svc_sock_setbufsize(svsk->sk_sock, SVC_UDP_DEFAULT_SOCKSIZE);
...
static void
svc_tcp_write_space(struct sock *sk)
{
	struct svc_sock *svsk;
	struct socket *sock;
...
svc_tcp_accept(struct svc_sock *svsk)
{
	...
	/* Set up peer address for TCP connection */
	memcpy(&svsk->sk_raddr, &sin, sizeof(sin));
...
svc_tcp_recvfrom(struct svc_rqst *rqstp)
{
	...
	if (svsk->sk_tcplen < 4) {
		...
		if (svsk->sk_reclen > (bufp->buflen << 2)) {
			printk(KERN_NOTICE "RPC: bad TCP reclen %d\n",
			       svsk->sk_reclen);
			goto err_delete;
		}
	}
 ...
 */
static int
svc_tcp_sendto(struct svc_sock *svsk, struct svc_buf *bufp)
{
	int sent = bufp->sent;

	struct iovec *iovp = bufp->iov;
	int nriov = bufp->nriov; 
	int tosend = (bufp->len << 2) - sent;
	...
	while (1) {
		sent = svc_sendto(svsk, bufp, iovp, nriov);
		if (sent < 0)
			break;
		...
		if (sent != tosend) {
			...
			iovp->iov_base = ((unsigned char *) iovp->iov_base) + 
					 sent;
			iovp->iov_len -= sent;
			tosend -= sent;
		} else {
...
svc_tcp_init(struct svc_sock *svsk)
{
	...
	if (sk->state == TCP_LISTEN) {
		...
		sk->write_space = svc_tcp_write_space;

		svsk->sk_reclen = 0;
		svsk->sk_tcplen = 0;
	}
	svc_sock_setbufsize(svsk->sk_sock, SVC_TCP_DEFAULT_SOCKSIZE);
...
svc_recv(struct svc_serv *serv, struct svc_rqst *rqstp, long timeout)
{
	...
	if (!(rqstp->rq_resbuf = svc_resbuf_alloc(serv))) {
...
out_error:
	svc_resbuf_free(rqstp->rq_resbuf);
	rqstp->rq_resbuf = NULL;
out_noresbuf:
...
static int
svc_empty_sendq(struct svc_sock *svsk)
{
	int ret = -EINVAL;
	struct svc_buf *resbufp;
	...
	if (test_and_set_bit(SK_SENDING, &svsk->sk_sendstate))
		return 0;
	...
	while ((resbufp = svsk->sk_sendq) != NULL) {
		...
		/* work with resbufp */
		clear_bit(SK_WSPACE, &svsk->sk_sendstate);
		set_bit(SK_INSEND, &svsk->sk_sendstate);
		ret = svsk->sk_sendto(svsk, resbufp);
		clear_bit(SK_INSEND, &svsk->sk_sendstate);
		spin_lock(&svsk->sk_sendlk);
		if (ret >= 0) {
			rpc_remove_list(&svsk->sk_sendq, resbufp);
			svc_resbuf_free(resbufp);
		...
		if ( ret != -EAGAIN)
			break;
		if (!test_bit(SK_WSPACE, &svsk->sk_sendstate)) 
			break;
	}
	spin_unlock(&svsk->sk_sendlk);
	clear_bit(SK_SENDING, &svsk->sk_sendstate);
	return 0;
}
...
svc_send(struct svc_rqst *rqstp)
{
	...
	spin_lock(&svsk->sk_sendlk);
	rpc_append_list(&svsk->sk_sendq, rqstp->rq_resbuf);
	rqstp->rq_resbuf = NULL;
	spin_unlock(&svsk->sk_sendlk);
...
svc_delete_socket(struct svc_sock *svsk)
{
	...
	*rsk = svsk->sk_list;
	...
	/*
	 * If the socket is not in use, we can close it now. If not we leave
	 * the cleanup for svc_sock_release().
	 */
	if (!svsk->sk_inuse) {
		spin_unlock_bh(&serv->sv_lock);
		svc_sock_close(svsk);
