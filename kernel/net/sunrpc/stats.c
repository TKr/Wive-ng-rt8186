...
kmem_cache_t *rpc_rbcachep = NULL;
...
static int
svc_rbcache_create(void)
{
	if (!(rpc_rbcachep = kmem_cache_create("rpc_rbcache",
					       sizeof (struct svc_buf), 0,
...
static int
svc_rbcache_destroy(void)
{
	if (rpc_rbcachep && kmem_cache_destroy(rpc_rbcachep)) {
		printk(KERN_WARNING "RPC: Unable to destroy rpc_rbcache (%p).\n"
...
static int __init init_sunrpc(void)
{
	...
	err = svc_rbcache_create();
	return err;
}

static void __exit exit_sunrpc(void)
{
#ifdef RPC_DEBUG
