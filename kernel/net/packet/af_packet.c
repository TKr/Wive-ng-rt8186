 ...
 *
 * Version:	$Id: af_packet.c,v 1.1.1.1 2005/03/09 04:54:42 rex Exp $
 *
			       ...
			       struct scm_cookie *scm)
{
	...
	err = -ENOBUFS;
	skb = sock_wmalloc(sk, len+dev->hard_header_len+MAX_SPEEDUP_BUFF, 0, GFP_KERNEL);
	 ...
	 */
	skb_reserve(skb,(dev->hard_header_len+MAX_SPEEDUP_BUFF)&~MAX_SPEEDUP_BUFF);
	skb->nh.raw = skb->data;
...
#ifdef  CONFIG_INET
#ifdef CONFIG_KMOD
			if (divert_ioctl_hook == NULL)
				request_module("dv");
#endif /* CONFIG_KMOD */
			if (divert_ioctl_hook != NULL)
				return(divert_ioctl_hook(cmd, (struct divert_cf *) arg));
			else
				return -ENOPKG;
#endif /* CONFIG_INET */
			return -ENOPKG;
