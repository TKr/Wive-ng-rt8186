...
char ipsec_tunnel_c_version[] = "RCSID $Id: ipsec_tunnel.c,v 1.196 2002/07/24 18:44:54 rgb Exp $";
...
skb_copy_expand(struct sk_buff *skb, int headroom, int tailroom, int priority)
{
	...
	KLIPS_PRINT(debug_tunnel & DB_TN_CROUT,
		    ...
		    "head=0p%p data=0p%p tail=0p%p end=0p%p end-head=%d tail-data=%d\n",
		    skb->head,
		    skb->data,
		    skb->tail,
		    skb->end,
				...
				if (pfkey_acquire(&ips) == 0) {
					...
					if (outgoing_said.spi==htonl(SPI_TRAPSUBNET)) {
						...
						if (ipsec_breakroute(&(hold_eroute.er_eaddr),
							...
							KLIPS_PRINT(debug_pfkey,
								    "klips_debug:ipsec_tunnel_start_xmit: "
								    "breakeroute should have failed.  first=0p%p, last=0p%p\n",
								    first, last);
							if(first != NULL) {
								kfree_skb(first);
							}
...
ipsec_tunnel_init_devices(void)
{
	...
	KLIPS_PRINT(1 || debug_tunnel & DB_TN_INIT,
		    ...
		    "creating and registering IPSEC_NUM_IF=%d devices, IFNAMSIZ=%d.\n",
		    IPSEC_NUM_IF, IFNAMSIZ);
#endif
