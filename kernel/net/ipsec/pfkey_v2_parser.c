 ...
 * for more details.
 *
 * RCSID $Id: pfkey_v2_parser.c,v 1.109 2002/07/26 08:48:31 rgb Exp $
 */
...
char pfkey_v2_parser_c_version[] = "$Id: pfkey_v2_parser.c,v 1.109 2002/07/26 08:48:31 rgb Exp $";
...
DEBUG_NO_STATIC int
pfkey_alloc_ipsec_sa(struct ipsec_sa** ips)
{
	int error = 0;
	if(*ips) {
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:pfkey_alloc_ipsec_sa: "
			    "ipsec_sa struct already allocated\n");
		SENDERR(EEXIST);
	}
	...
	KLIPS_PRINT(debug_pfkey,
		    "klips_debug:pfkey_alloc_ipsec_sa: "
		    "allocated ips_ref=%u, SAref[%u]->entry[%u].\n",
		    (*ips)->ips_ref,
		    SAref2SArefTable((*ips)->ips_ref),
		    SAref2SArefEntry((*ips)->ips_ref));
#endif
errlab:
	return(error);
}
...
pfkey_x_grpsa_parse(struct sock *sk, struct sadb_ext **extensions, struct pfkey_extracted_data* extr)
{
	...
	if(ips1p == NULL) {
		...
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:pfkey_x_grpsa_parse: "
			    "reserved ipsec_sa for SA: %s not found.  Call SADB_ADD/UPDATE first.\n",
			    sa_len1 ? sa1 : " (error)");
		SENDERR(ENOENT);
	}
	if(extr->ips2) { /* GRPSA */
				...
				struct sadb_msg **pfkey_reply)
{
	...
	if((error = pfkey_alloc_ipsec_sa(&(extr.ips)))) {
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:pfkey_msg_interp: "
			    "something's really wrong, extr.ips=0p%p should be NULL.\n",
			    extr.ips);
		SENDERR(-error);
	}
	...
	KLIPS_PRINT(debug_pfkey,
		    "klips_debug:pfkey_msg_interp: "
		    "parsing message type %d with msg_parser 0p%p.\n",
		    pfkey_msg->sadb_msg_type,
