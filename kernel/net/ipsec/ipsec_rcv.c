...
char ipsec_rcv_c_version[] = "RCSID $Id: ipsec_rcv.c,v 1.108 2002/07/24 18:44:54 rgb Exp $";
...
#endif /* PROTO_HANDLER_SINGLE_PARM */
{
	...
	struct ipsec_sa* ipsprev = NULL;	/* previous SA from outside of packet */
	...
	do {
		...
		if(proto == IPPROTO_AH) {
			...
			if (ahhlen != sizeof(struct ah)) {
				KLIPS_PRINT(debug_rcv & DB_RX_INAU,
					    "klips_debug:ipsec_rcv: "
					    "bad authenticator length %ld, expected %d from %s\n",
					    (long)(ahhlen - ((caddr_t)(ahp->ah_data) - (caddr_t)ahp)),
					    AHHMAC_HASHLEN,
					    ipaddr_txt);
				if(stats) {
					stats->rx_errors++;
		...
		*/
		...
		if (proto == IPPROTO_COMP) {
			...
			if(sysctl_ipsec_inbound_policy_check
			   && ((ipsp == NULL)
				...
				/* next line is a workaround for peer
				   ...
				   && (ipsp->ips_encalg != ntohl(said.spi)) 
				       ))) {
				char sa2[SATOA_BUF];
				size_t sa_len2 = 0;
			...
			if (ipsp != NULL) {
				ipsp->ips_comp_ratio_cbytes += ntohs(ipp->tot_len);
			...
			if (ipsp != NULL) {
				ipsp->ips_comp_ratio_dbytes += ntohs(ipp->tot_len);
			}
		...
		if(sysctl_ipsec_inbound_policy_check) {
			...
			if(ipsp->ips_onext) {
				if(ipsprev != ipsp->ips_onext) {
					spin_unlock(&tdb_lock);
					KLIPS_PRINT(debug_rcv,
						    "klips_debug:ipsec_rcv: "
						    "unexpected SA:%s: does not agree with ips->onext policy, dropped.\n",
						    sa_len ? sa : " (error)");
					if(stats) {
						stats->rx_dropped++;
					}
		...
		if(ipsec_lifetime_check(&ipsp->ips_life.ipl_bytes,   "bytes", sa,
					ipsec_life_countbased, ipsec_incoming, ipsp) == ipsec_life_harddied ||
		   ipsec_lifetime_check(&ipsp->ips_life.ipl_addtime, "addtime",sa,
					ipsec_life_timebased,  ipsec_incoming, ipsp) == ipsec_life_harddied ||
		   ipsec_lifetime_check(&ipsp->ips_life.ipl_addtime, "usetime",sa,
					ipsec_life_timebased,  ipsec_incoming, ipsp) == ipsec_life_harddied ||
		...
		if (!ipsec_checkreplaywindow(ipsp, replay)) {
			ipsp->ips_errs.ips_replaywin_errs += 1;
			spin_unlock(&tdb_lock);
		...
		if(ipsp->ips_replaywin && !replay /* !ipsp->ips_replaywin_lastseq */) {
			ipsec_sa_delchain(ipsp);
			spin_unlock(&tdb_lock);
			KLIPS_PRINT(debug_rcv,
				    "klips_debug:ipsec_rcv: "
		...
		if (!ipsec_updatereplaywindow(ipsp, replay)) {
			ipsp->ips_errs.ips_replaywin_errs += 1;
			spin_unlock(&tdb_lock);
			KLIPS_PRINT(debug_rcv & DB_RX_REPLAY,
				    "klips_debug:ipsec_rcv: "
