 ...
 * for more details.
 *
 * RCSID $Id: pfkey_v2_ext_process.c,v 1.3 2002/07/24 18:44:54 rgb Exp $
 */
...
char pfkey_v2_ext_process_c_version[] = "$Id: pfkey_v2_ext_process.c,v 1.3 2002/07/24 18:44:54 rgb Exp $";
...
pfkey_sa_process(struct sadb_ext *pfkey_ext, struct pfkey_extracted_data* extr)
{
	...
	case SADB_EXT_SA:
		...
		break;
	case SADB_X_EXT_SA2:
		if(pfkey_alloc_ipsec_sa(&(extr->ips2)) == ENOMEM) {
			SENDERR(ENOMEM);
		}
		ipsp = extr->ips2;
		break;
