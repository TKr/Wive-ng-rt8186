 ...
 * for more details.
 *
 * RCSID $Id: ipsec_sa.c,v 1.13 2002/07/29 03:06:20 mcr Exp $
 *
...
struct SArefSubTable* SAref[SA_REF_MAINTABLE_SIZE];
struct SArefTableEntry * SArefFreeList = NULL;
struct SArefTableEntry * SArefFreeListTail = NULL;

int
ipsec_SArefSubTable_alloc(unsigned table)
{
	unsigned i;
	struct SArefSubTable* SArefsub;

	KLIPS_PRINT(1|debug_xform,
		    "klips_debug:ipsec_SArefSubTable_alloc: "
		    "allocating table=%u.\n",
		    table);

	/* allocate another sub-table */
	SArefsub = vmalloc(SA_REF_SUBTABLE_SIZE * sizeof(struct SArefTableEntry));
	if(SArefsub == NULL) {
	...
	SAref[table] = SArefsub;

	/* point the end of the free list at the first element of the
	 * new sub-table */
	if(SArefFreeList == NULL) {
		SArefFreeList = &(SArefsub->entry[0]);
	} else {
		SArefFreeListTail->SA = (struct ipsec_sa*)&(SArefsub->entry[0]);
	}
	/* initialise each element to point to the next for the freelist  and set the refnum */
	for(i = 0; i < SA_REF_SUBTABLE_SIZE - 1; i++) {
		SArefsub->entry[i].SA = (struct ipsec_sa*)&(SArefsub->entry[i + 1]);
		SArefsub->entry[i].SAref = table * SA_REF_SUBTABLE_SIZE + i;
	}
	SArefsub->entry[SA_REF_SUBTABLE_SIZE - 1].SA = NULL;
	SArefsub->entry[SA_REF_SUBTABLE_SIZE - 1].SAref = (table + 1) * SA_REF_SUBTABLE_SIZE - 1;

	SArefFreeListTail = &(SArefsub->entry[SA_REF_SUBTABLE_SIZE - 1]);
...
ipsec_sadb_init(void)
{
	...
	KLIPS_PRINT(1|debug_xform,
	    	...
	    	"initialising main table.\n");
	for(i = 0; i < SA_REF_MAINTABLE_SIZE; i++) {
		printk("%u", i%10);
		SAref[i] = NULL;
	}
	printk("\n");
...
ipsec_sa_alloc(void)
{
	...
	KLIPS_PRINT(debug_xform,
		    "klips_debug:ipsec_sa_alloc: "
		    "allocated ipsec_sa struct=0p%p.\n", ips);

	memset((caddr_t)ips, 0, sizeof(*ips));
	ips->ips_ref = ipsec_SAref_alloc()->SAref;

	return(ips);
}

struct SArefTableEntry *
ipsec_SAref_alloc(void)
{
	int error = 0;
	unsigned i;
	struct SArefTableEntry * SArefEntry;

	if(SArefFreeList != NULL) {
		KLIPS_PRINT(debug_xform,
			    "klips_debug:ipsec_SAref_alloc: "
			    "allocating SAref=%u, table=%u, entry=%u.\n",
			    SArefFreeList->SAref,
			    SAref2SArefTable(SArefFreeList->SAref),
			    SAref2SArefEntry(SArefFreeList->SAref));
		
		SArefEntry = SArefFreeList;
		SArefFreeList = (struct SArefTableEntry*)(SArefFreeList->SA);
		SArefEntry->SA = NULL;
		return SArefEntry;
	}

	/* check if there is space in the main table */
	for(i = 1; i < SA_REF_MAINTABLE_SIZE; i++) {
		if(SAref[i] == NULL) {
			break;
		}
	}
	if(i == SA_REF_MAINTABLE_SIZE) {
		return NULL;
	}
	
	/* allocate another sub-table */
	error = ipsec_SArefSubTable_alloc(i);
	if(error) {
		return NULL;
	}

	/* allocate a new entry from the first of the list */
	SArefEntry = &(SAref[i]->entry[0]);
	KLIPS_PRINT(debug_xform,
		    "klips_debug:ipsec_SAref_alloc: "
		    "allocating SAref=%u, table=%u, entry=%u.\n",
		    SArefEntry->SAref,
		    SAref2SArefTable(SArefEntry->SAref),
		    SAref2SArefEntry(SArefEntry->SAref));

	/* point the free list at the second element of the new sub-table */
	SArefFreeList = &(SAref[i]->entry[1]);
...
ipsec_sa_put(struct ipsec_sa *ips)
{
	...
	KLIPS_PRINT(debug_xform,
		    ...
		    "ipsec_sa SA:%s reference count decremented.\n",
		    sa_len ? sa : " (error)");
...
ipsec_sadb_cleanup(__u8 proto)
{
 ...
 errlab:
	...
	/* walk the free list first to remove those entries from being
	   confused with real entries. */
	KLIPS_PRINT(1|debug_xform,
		    "klips_debug:ipsec_sadb_cleanup: "
		    "removing all SArefFreeList entries from circulation.");
#if 0
	while(SArefFreeList != NULL) {
		KLIPS_PRINT(debug_xform,
			    "klips_debug:ipsec_sadb_cleanup: "
			    "removing SAref=%u, table=%u, entry=%u from circulation.\n",
			    SArefFreeList->SAref,
			    SAref2SArefTable(SArefFreeList->SAref),
			    SAref2SArefEntry(SArefFreeList->SAref));
		printk(".");
	...
	for(i = 0; i < SA_REF_MAINTABLE_SIZE; i++) {
		printk("%u", i);
		for(j = 0; i < SA_REF_SUBTABLE_SIZE; j++) {
			ipsec_sa_delchain(SAref[i]->entry[j].SA);
			SAref[i]->entry[j].SA = NULL;
			printk("%u", j%10);
		}
		KLIPS_PRINT(debug_xform,
			    "klips_debug:ipsec_sadb_cleanup: "
...
#if 0
	/* add my SAref to the end of the SArefFreeList */
	KLIPS_PRINT(1|debug_xform,
		    ...
		    SAref2SArefTable(SArefFreeList->SAref),
		    SAref2SArefEntry(SArefFreeList->SAref));
	SArefFreeListTail->SA = (struct ipsec_sa*)&(SAref2SA(SA2SAref(ips)));
	SAref2SA(SA2SAref(ips)) = NULL;
	SArefFreeListTail = (struct SArefTableEntry *)(SArefFreeListTail->SA);
#endif
