...
char ipsec_proc_c_version[] = "RCSID $Id: ipsec_proc.c,v 1.14 2002/07/26 08:48:31 rgb Exp $";
...
#ifdef CONFIG_IPSEC_DEBUG
			if(debug_xform) {
			len += sprintf(buffer + len, " ref=%d reftable=%d refentry=%d",
				       sa_p->ips_ref,
				       SAref2SArefTable(sa_p->ips_ref),
				       SAref2SArefEntry(sa_p->ips_ref));
			}
#endif /* CONFIG_IPSEC_DEBUG */
