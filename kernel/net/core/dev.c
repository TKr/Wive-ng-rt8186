...
#include <linux/wireless.h>    /* Note : will define standart WIRELESS_EXT */
#include <linux/iw_handler.h>  /* Note : will define api WIRELESS_EXT */
#include <linux/ieee802_mib.h> /* Note : will define mibs for rtl8186 */
 ...
 * during overflow.
 * netdev_max_backlog specifies the maximum length of the input queues for the processors.
 * The default value is 300 (packets). Linux has to wait up to scheduling time to flush buffers
 * (due to bottom half mechanism). Therefore, this value can limit the network bandwidth when
 * receiving packets as follows: 
 * 300       *  	    100     =      30,000
...
int netdev_max_backlog = 1500; // after smart bit test, 800-1500 is optimized 
