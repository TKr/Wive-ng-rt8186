 ...
 *
 *	Version: $Id: igmpv3.c,v 1.1 2007/10/09 12:21:44 jiunming Exp $
 *
 *	Authors:
	...
	int sfcount, __u32 *psfsrc, int delta)
{
	...
	if (!delta) {
		...
		if (!pmc->sfcount[sfmode])
			goto out_unlock;//v35 //return -EINVAL;
		pmc->sfcount[sfmode]--;
	}
	err = 0;
	for (i=0; i<sfcount; i++) {
	...
	ip_mreq_source *mreqs, int ifindex)
{
	...
	if (pmc->sflist) {
		...
		ip_mc_add_src(in_dev, &mreqs->imr_multiaddr, omode, 0, 0, 0);//v35
	...
	if (!add) {
		...
		for (i=0; i<psl->sl_count; i++) {
			rv = memcmp(&psl->sl_addr[i], &mreqs->imr_sourceaddr, //v35
				sizeof(__u32));
			if (rv == 0) //v35
				break;
		}
		if (rv)	/* source not found */ //v35
			goto done;
