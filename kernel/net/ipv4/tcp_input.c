int sysctl_tcp_delack_strategy=10;    /* default: no immediate ACKs  */

#define TOP1 0x80000000
#define TOP2 0xc0000000
#define TOP3 0xe0000000
#define TOP4 0xf0000000
#define TOP5 0xf8000000
#define TOP6 0xfc000000
#define TOP7 0xfe000000
#define TOP8 0xff000000
#define TOP9 0xff800000

inline unsigned long delay_this_ack(void)
{
/*
   This uses a simple random number generator to decide 
   Probability of immediate ACKs is chosen by sysctl_tcp_delack_strategy
   Author: Josip Loncaric <josip@icase.edu> October 28, 1999
*/
    static unsigned long lseed = 47114711;
    unsigned long nseed = lseed * 1103515245 + 12345;
    lseed = nseed;
    switch (sysctl_tcp_delack_strategy) {
	case 0:	return(0); break;		/* all immediate ACKs */
	case 1: return(lseed & TOP1); break;	/* 1/2 */
	case 2: return(lseed & TOP2); break;	/* 1/4 */
	case 3: return(lseed & TOP3); break;	/* 1/8, a good choice */
	case 4: return(lseed & TOP4); break;	/* 1/16 */
	case 5: return(lseed & TOP5); break;	/* 1/32 */
	case 6: return(lseed & TOP6); break;	/* 1/64 */
	case 7: return(lseed & TOP7); break;	/* 1/128 */
	case 8: return(lseed & TOP8); break;	/* 1/256 */
	case 9: return(lseed & TOP9); break;	/* 1/512 */
	default: return(1); break;		/* no immediate ACKs */
    }
}
 ...
 *
 * Version:	$Id: tcp_input.c,v 1.1.1.1 2004/07/28 06:27:37 ysc Exp $
 *
...
void tcp_enter_loss(struct sock *sk, int how)
{
	...
	struct sk_buff *skb;
	int cnt = 0;
	/* PS: Idea of F-RTO is as follows: do not start retransmitting
        * immediately after RTO. Instead send two new segments to see
        * whether there are really data lost. However, the segment that
        * triggered RTO is always retransmitted.
        *
        * When F-RTO is enabled, enter_loss can be called two times for
        * a single RTO: On first pass it does not yet mark packets lost,
        * but if acknowledgements indicate so, enter_loss is called second
        * time for the sender to start retransmitting.
        * postpone_loss is true, if F-RTO is enabled and we are on
        * the first pass.
        */
        int postpone_loss = (tcp_use_frto(sk) && !tp->frto_counter);
	...
	if (!tp->frto_counter) {
       ...
       if (!tcp_use_frto(sk)) {
               /* PS: When F-RTO is enabled, cwnd is adjusted
                * in tcp_process_frto */
               tp->snd_cwnd = 1;
	...
	/* PS: clear_retrans only once, if RTO is used */
	if (!tp->frto_counter)
	...
	for_retrans_queue(skb, sk, tp) {
		...
		if (!tp->frto_counter && (tp->frto_highmark == 0 || after(TCP_SKB_CB(skb)->end_seq, tp->frto_highmark))) {
			tp->frto_highmark = TCP_SKB_CB(skb)->end_seq;
		}

               /* PS: This clears retrans flags. When F-RTO is used, it should
		* be done only once per RTO
                */
               if (!tp->frto_counter) {
	       ...
	       if (!(TCP_SKB_CB(skb)->sacked&TCPCB_SACKED_ACKED) || how) {
		       ...
		       /* PS: If F-RTO is enabled, mark those packets lost
			* which were outstanding when RTO occurred. This is
			* done only on the second pass.
			*/
		       if (!tcp_use_frto(sk) ||
			   (tp->frto_counter &&
...
static void tcp_process_frto(struct sock *sk, u32 prior_snd_una)
{
	...
	if (tp->frto_counter == 2) {
		if (tp->snd_una == prior_snd_una) {
			/* RTO was caused by a loss. Because two round-trips
			 * have passed after RTO, cwnd is adjusted to let
			 * three packets out. */
			tcp_enter_loss(sk, 0);
			tp->snd_cwnd = tcp_packets_in_flight(tp) + 3;
			tp->undo_marker = 0;
		} else {
			/* A non-retransmitted segment is acknowledged and
			 * RTO was very likely spurious. However, we'll be
			 * careful and reduce the congestion window to half
			 */
			tp->snd_cwnd = min(tp->snd_cwnd, tp->snd_ssthresh);
		}
	}

	/* F-RTO affects on two ACKs following RTO. On third ACK the TCP
	 * behavor is back to normal.
	 */
	if (tp->frto_counter) tp->frto_counter++;
	if (tp->frto_counter > 2) tp->frto_counter = 0;
}
...
static __inline__ void __tcp_ack_snd_check(struct sock *sk, int ofo_possible)
{
	...
	if ((tp->rcv_nxt - tp->rcv_wup) > tp->ack.rcv_mss ||
	    ...
	    /* ...or TCP_NODELAY is set and we do not delay this ACK -- Josip */
	    (tp->nonagle == 1 && !delay_this_ack()) ) {
		/* Then ack it now */
		tcp_send_ack(sk);
	} else {
