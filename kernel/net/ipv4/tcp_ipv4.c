 ...
 *
 * Version:	$Id: tcp_ipv4.c,v 1.1.1.1 2004/07/28 06:27:37 ysc Exp $
 *
...
#ifdef CONFIG_IP_STEALTH
	/* We can avoid sending an RST in response to a SYN on a closed port.
	   This slows down port scans without the need of a firewall, and
	   without the firewall disagreeing with all any process. Also, out
	   of sync packets are still answered.
	*/
	if (sysctl_tcp_restrict_rst && th->syn && !th->ack)
		return;
#endif
