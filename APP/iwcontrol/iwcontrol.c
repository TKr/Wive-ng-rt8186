...
int parsing_var_pid(Dot1x_RTLDListener *auth, Dot1x_RTLDListener *autoconf)
{
       DIR *dir;
        struct dirent *next;
        int i;
...
//-------------------------------------------------------------
	...
	if(parsing_var_pid(RTLDListenerAuth, RTLDListenerAutoconf) < 0){
	...
	if(link_auth){
		for(i=0 ; i < link_auth; i++){
			sprintf(auth_fifo, DAEMON_FIFO, RTLDListenerAuth[i].wlanName);
			iw_message(MESS_DBG_CONTROL,"open auth fifo %s\n", auth_fifo);
			iw_init_fifo(&RTLDListenerAuth[i], auth_fifo);
		}
	}
	if(link_iapp){
	...
	if(link_autoconf){
		for(i=0 ; i < link_autoconf; i++){
			sprintf(autoconf_fifo, AUTOCONFIG_FIFO, RTLDListenerAutoconf[i].wlanName);
			iw_message(MESS_DBG_CONTROL,"open autoconfig fifo %s\n", autoconf_fifo);
			iw_init_fifo(&RTLDListenerAutoconf[i], autoconf_fifo);
		}
	}
#endif		
	...
	if (!poll)	// david
	{
		...
		char  dev_name[20] ;
		for(i=0; i< wlan_num ;i++){
			sprintf(dev_name, "/dev/wl_chr%d", i);
